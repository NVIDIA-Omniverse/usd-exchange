...
- **`tools`** — required. Top-level YAML flow-list of agent tool names that the skill body actually invokes — loading reference files, running shell commands, editing user code, etc. Standard tool names: `Read`, `Write`, `Edit`, `Shell`, `Grep`, `Glob`. Every tool referenced in the body must appear in the list, and every listed tool must be referenced by the body (no dead declarations). Keep this field at the top level of the frontmatter (between `license:` and `metadata:`) — do not nest it under `metadata:`.
